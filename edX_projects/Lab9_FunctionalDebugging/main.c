...
void Delay50ms(void); //Delay by 50ms
...
int main(void){  unsigned long i,last,now;
  ...
  unsigned long previousData = 0xFFFFFFFF; // Initialize to a value that PortF will never have
  unsigned int index = 0; // Index for Data array

    while(1) {
        unsigned long switches = GPIO_PORTF_DATA_R & 0x11; // Read PF4 and PF0
        if (!(switches & 0x10) || !(switches & 0x01)) { // If either switch is pressed (logic low)
            GPIO_PORTF_DATA_R ^= 0x02; // Toggle PF1
            Delay50ms(); // 50ms delay to achieve 10 Hz blinking
        } else {
            GPIO_PORTF_DATA_R &= ~0x02; // Make sure LED is off if neither switch is pressed
        }

        // Record only on change
        unsigned long currentData = GPIO_PORTF_DATA_R & 0x13; // Mask for PF4, PF1, PF0
        if (currentData != previousData && index < 50) {
...
// pink     R-B    0x06
void Delay50ms(void) {
    // Assuming SysTick is already initialized to use the system clock
    NVIC_ST_RELOAD_R = 800000 - 1; // 50ms delay at 16 MHz
    NVIC_ST_CURRENT_R = 0; // Any write to CURRENT clears it
